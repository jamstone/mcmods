package mods.jamstone.pathfinding.minecart;
...
import mods.jamstone.pathfinding.Weight;
...
public class MinecartPathThread extends Thread {
    ...
    private boolean hasChunkBeenPopulatedAt(World worldObj, int blockX, int blockZ) {
        int chunkX = blockX >> 4;
        int chunkZ = blockZ >> 4;
        WorldServer world = (WorldServer) worldObj;
        File file = world.getChunkSaveLocation();
        ...
        try {
            nbttagcompound = CompressedStreamTools.read(datainputstream).getCompoundTag("Level");
            return nbttagcompound.getBoolean("TerrainPopulated");
        } catch (IOException e) {
    ...
    public boolean isPassableBlock(World worldObj, int ix, int iy, int iz) {
        if (!worldObj.checkChunksExist(ix, iy, iz, ix, iy, iz)) {
//        while (!hasChunkBeenPopulatedAt(worldObj, ix, iz)) {
            try {
                ArrayList<ChunkCoordIntPair> chunksNeeded = new ArrayList<ChunkCoordIntPair>();
                for (int i = ix >> 4; Math.abs((ix >> 4) - i) < 5; i += (int) Math.copySign(1, targetNode.x - ix)) {
                    for (int j = iz >> 4; Math.abs((iz >> 4) - j) < 5; j += (int) Math.copySign(1, targetNode.z - iz)) {
                        chunksNeeded.add(new ChunkCoordIntPair(i, j));
                    }
                }
...
/*        if (plane != Direction.UNE) {
    ...
    private AStarNode jump(int probeX, int probeY, int probeZ, int parentX,
        ...
        if (true || dist > AStarStatic.getDistanceBetween(0, 0, 0, 1, 1, 1) || dist > MAX_SKIP_DISTANCE // setting this to 0 effectively turns the algorithm into A* "Every point is a jump point"
