...
public class MinecartPathPlanner {
    private MinecartPathWorker worker;
    ...
    public MinecartPathPlanner(World world, IMinecartPathedEntity ent) {
        worldObj = world;
        pathedEntity = ent;
        isJPS = true;
    }
...
//        }
    ...
    public synchronized void getPath(AStarNode start, AStarNode end, AxisAlignedBB boundingBox) {
        ...
        worker = new MinecartPathWorker(this);//isJPS ? new AStarWorkerJPS(this) : new AStarWorker(this);
        worker.setup(worldObj, start, end, boundingBox);
        worker.start();
    }

    public void onFoundPath(MinecartPathWorker aStarWorker, ArrayList<AStarNode> result) {
