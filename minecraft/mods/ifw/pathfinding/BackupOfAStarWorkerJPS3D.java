...
//
//public class BackupOfAStarWorkerJPS3D extends AStarWorker {
...
//
//	ArrayList<AStarNode> jumppoints = new ArrayList<AStarNode>();
//
//	/**
...
//
//	public BackupOfAStarWorkerJPS3D(AStarPathPlanner creator) {
//		super(creator);
...
//
//		ArrayList<Direction> successors = node.parent == null ? dumbNeighbourDirections(
//				node.x, node.y, node.z) : smartNeighbourDirections(node.x,
//				node.y, node.z, node.getDirectionFrom(node.parent));
//
...
//		}
//
//		successors = neighboursD1;
//		successors.addAll(neighboursD2);
//		successors.addAll(neighboursD3);
//		if (directionOfTrunk != null)
//			successors.add(directionOfTrunk);
//
//		for (Direction dir : successors) {
//
//			AStarNode jumpPoint = jump(node.x + dir.x, node.y + dir.y, node.z
//					+ dir.z, node.x, node.y, node.z, currentNode);
//
...
//
//		// this will find the absolute shortest path, but since we're weighting
//		// out heuristic anyways, there's not much point is caring about that.
//		// seems to add a lot of extra processing to the search.
