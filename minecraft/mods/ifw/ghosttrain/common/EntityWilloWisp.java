...
//        despawnEntity();
        ...
        if (this.ridingEntity != null) {
            System.out.println("I AM IN A MINECART YAY WAIT NO :C");
        }
        ...
        if (ghostTrain != null) {
            if (path == null) {
                ...
                if (worldObj.getBlockMaterial((int) Math.floor(ghostTrain.posX), (int) Math.floor(ghostTrain.posY), (int) Math.floor(ghostTrain.posZ)) != Material.circuits
                    ...
                    //                   ghostTrain.setPosition(path.get(path.size() - 1).x+0.5, path.get(path.size() - 1).y + 1, path.get(path.size() - 1).z+0.5);
                } else {
    ...
    private void clearAddedBlocks() {
        if (!added.isEmpty() && lastAs != null) {
            ...
            while (lastAs.getDistanceTo(cc.posX, cc.posY, cc.posZ) > AStarStatic.getDistanceBetween(0, 0, 0, 16, 16, 16) || path == null) {
                added.removeFirst();
